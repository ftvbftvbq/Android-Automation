 ...
 */
public class CancelFinal_test extends ActivityInstrumentationTestCase2<HomeActivity> {
    private Solo solo;
    ...
    public void test_CancelFinal() throws Exception {
        LogUtil Logger = new LogUtil();
        Logger.configLog("MXD_CancelFinal_log4j.log");
        ReadExcel TestData = new ReadExcel();
        TestData.getTestDataFile("/storage/emulated/0/mxdCancelFinalTestData.csv");

        TestResults = new String[TestData.rowCount];
        for (int i = 1; i < TestData.rowCount; i++) {
            Logger.gLogger.debug("数据库取消终审");
            ...
            sqlu.UpdateInborrowsArchive("update borrows_archive set is_audit=1,is_confirm=5 where bo_id="+sqlu.id);
        ...
        TestData.writeResults(TestResults, "mxdCancelFinalTestResult.txt");
    }
