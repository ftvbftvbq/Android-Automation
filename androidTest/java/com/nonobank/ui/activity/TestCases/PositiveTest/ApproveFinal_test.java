 ...
 */
public class ApproveFinal_test extends ActivityInstrumentationTestCase2<HomeActivity> {
    private Solo solo;
    ...
    public void test_ApproveStepFinal () throws Exception {
        LogUtil Logger = new LogUtil();
        Logger.configLog("MXD_ApproveStepFinal_log4j.log");
        ...
        for (int i = 1; i < TestData.rowCount; i++) {
            Logger.gLogger.debug("数据库操作终审通过");
            ...
            sqlu.UpdateInborrowsArchive("update borrows_archive set is_audit=1, is_confirm=1 where bo_id="+sqlu.id);
            solo.sleep(20000);
            ...
            if (solo.waitForWebElement(By.id("runStep3"),5000,false) == true) {
                TestResults[i] = "Pass";
            }else if (solo.waitForWebElement(By.id("runStep3"),5000,false) == false){
                TestResults[i] = "Failed";
            }
        }
        TestData.writeResults(TestResults, "mxdApproveStepFinalTestResult.txt");
    }
