 ...
 */
public class RejectTail_test extends ActivityInstrumentationTestCase2<HomeActivity> {
    private Solo solo;
    ...
    public void test_RejectTail () throws Exception {
        LogUtil Logger = new LogUtil();
        Logger.configLog("MXD_RejectTail_log4j.log");
        ReadExcel TestData = new ReadExcel();
        TestData.getTestDataFile("/storage/emulated/0/mxdRejectTailTestData.csv");

        TestResults = new String[TestData.rowCount];
        for (int i = 1; i < TestData.rowCount; i++) {
            Logger.gLogger.debug("数据库拒绝初审");
            ...
            sqlu.UpdateInborrowsArchive("update borrows_archive set is_audit=2 where bo_id="+sqlu.id);
        ...
        TestData.writeResults(TestResults, "mxdRejectTailTestResult.txt");
    }
